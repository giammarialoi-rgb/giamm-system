...
var DI_VERSION = "1.0.0";
var DI_MAX_BYTES = 12 * 1024 * 1024;

var DI_PIPELINE_STAGES = [
...
function createSourceRef(partial = {}) {
  return {
...
function createEmptyDocumentIR(meta = {}) {
  return {
    version: DI_VERSION,
    document: {
 ...
 */
function detectFormat(bytes, filename = "", mimeType = "") {
 ...
 */
function expandMergesToGrid(rawRows, merges = []) {
 ...
 */
function reconstructTablesFromSheet(sheet, fileId = null) {
 ...
 */
function buildIRFromWorkbook(structured, filename, detectMeta = {}) {
 ...
 */
function reconstructTablesFromDocxXml(xml, fileId = null) {
  const tables = [];
 ...
 */
function reconstructTablesFromPositionedText(items, page = 1, fileId = null) {
 ...
 */
function pdfNeedsOcr(text, pageCount = 1) {
 ...
 */
async function runLocalOcr(imageSource, options = {}) {
...
function classifyDocumentFromSignals(sig = {}) {
 ...
 */
function validateDocumentSemantics(canonical) {
  const issues = [];
 ...
 */
function flagInferredSets(exercise, { inferred = false } = {}) {
 ...
 */
function searchDocumentIR(ir, query) {
 ...
 */
function diffCanonicalPrograms(a, b) {
...
function buildImportSummary(ir, canonical, validation) {
...
function enrichFoodMappingConfidence(nutrition) {
...
function enrichSupplementMappingConfidence(supp) {
...
function extractExcelStructuredForApi(workbook, XLSXlib) {
...
var TECHNIQUE_PATTERNS = Object.freeze([
 ...
 */
function parseDeclaredSetCount(text) {
...
function parseDeclaredReps(text) {
  const str = String(text || '');
  const serieIt = str.match(/\d{1,2}\s*(?:serie|sets?)\s*(?:[xX*\u00d7]|da|di|:)?\s*(\d+(?:[\-\u2013\/]\d+)?|AMRAP|MAX)\b/i);
...
function detectTechniquesFromText(...parts) {
 ...
 */
function resolveTargetSetCount(exercise) {
  const ex = exercise || {};
...
function reconcileSetRows(exercise, mapRow) {
...
function parseCompoundSchemes(text) {
  const str = String(text || '');
...
function parseSpaceLadder(text) {
...
function parseLiteralScheme(text) {
  const str = String(text || '')
    .replace(/#\s*\d+\b/g, ' ');
  if (!str.trim()) return null;
...
function extractPrescriptionsFromText(rawText) {
 ...
 */
function resolvePrescription(ex) {
  const e = ex || {};
...
function enforceExercisePrescription(ex, prescriptionOpt) {
  const e = ex || {};
 ...
 */
function applyPrescriptionsToProgram(program, rawText) {
...
function enforceAllPrescriptions(program) {
  if (!program || !Array.isArray(program.weeks)) return program;
  for (const week of program.weeks) {
    for (const session of (week.sessions || week.days || [])) {
      for (const ex of (session.exercises || session.rows || [])) {
...
function enrichTherapyMedications(therapy) {
...
{ DRUG_CATALOG, matchDrug, enrichTherapyMedications };
...
function searchSupplementCatalog(query, opts = {}) {
...
{ SUPPLEMENT_CATALOG, SUPPLEMENT_CATEGORIES, searchSupplementCatalog };
...
// === FOOD CATALOG ===
/** GENERATO da tools/build_food_catalog.mjs a partire da food-staples.json: non modificare a mano. */
var FOOD_CATALOG = [{"id":"food_Bevande_0","name":"Latte intero","name_en":"Whole milk","aliases":["Latte intero","Whole milk","latte intero","whole milk"],"category":"Bevande","kcal":61,"pro":3.2,"carb":4.8,"fat":3.3,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Bevande_1","name":"Latte parzialmente scremato","name_en":"Semi-skimmed milk","aliases":["Latte parzialmente scremato","Semi-skimmed milk","latte parzialmente scremato","semi-skimmed milk"],"category":"Bevande","kcal":46,"pro":3.4,"carb":4.8,"fat":1.6,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Bevande_2","name":"Latte scremato","name_en":"Skim milk","aliases":["Latte scremato","Skim milk","latte scremato","skim milk"],"category":"Bevande","kcal":34,"pro":3.4,"carb":5,"fat":0.1,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Bevande_3","name":"Latte di soia","name_en":"Soy milk","aliases":["Latte di soia","Soy milk","latte di soia","soy milk"],"category":"Bevande","kcal":33,"pro":2.9,"carb":1.2,"fat":1.8,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Bevande_4","name":"Latte di mandorla","name_en":"Almond milk","aliases":["Latte di mandorla","Almond milk","latte di mandorla","almond milk"],"category":"Bevande","kcal":24,"pro":0.6,"carb":1.5,"fat":1.8,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_Bevande_5","name":"Latte di riso","name_en":"Rice milk","aliases":["Latte di riso","Rice milk","latte di riso","rice milk"],"category":"Bevande","kcal":47,"pro":0.3,"carb":9.2,"fat":1,"serving":"100g","unit":"g","rank":5,"source":"nurvan_staple_v3"},{"id":"food_Bevande_6","name":"Succo d'arancia","name_en":"Orange juice","aliases":["Succo d'arancia","Orange juice","succo d'arancia","orange juice"],"category":"Bevande","kcal":45,"pro":0.7,"carb":10.4,"fat":0.2,"serving":"100g","unit":"g","rank":6,"source":"nurvan_staple_v3"},{"id":"food_Bevande_7","name":"Bevanda proteica ready","name_en":"Ready protein shake","aliases":["Bevanda proteica ready","Ready protein shake","bevanda proteica ready","ready protein shake"],"category":"Bevande","kcal":80,"pro":15,"carb":3,"fat":1,"serving":"100g","unit":"g","rank":7,"source":"nurvan_staple_v3"},{"id":"food_extra_15","name":"Latte","name_en":"Milk / Leche / Milch / Lait / Молоко","aliases":["Latte","Milk / Leche / Milch / Lait / Молоко","latte","milk / leche / milch / lait / молоко"],"category":"Bevande","kcal":61,"pro":3.2,"carb":4.8,"fat":3.3,"serving":"100g","unit":"g","rank":115,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_0","name":"Riso basmati","name_en":"Basmati rice","aliases":["Riso basmati","Basmati rice","riso basmati","basmati rice"],"category":"Carboidrati","kcal":360,"pro":7,"carb":79,"fat":0.6,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_1","name":"Riso integrale","name_en":"Brown rice","aliases":["Riso integrale","Brown rice","riso integrale","brown rice"],"category":"Carboidrati","kcal":370,"pro":7.5,"carb":77,"fat":2.7,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_2","name":"Riso jasmine","name_en":"Jasmine rice","aliases":["Riso jasmine","Jasmine rice","riso jasmine","jasmine rice"],"category":"Carboidrati","kcal":365,"pro":7,"carb":80,"fat":0.6,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_3","name":"Avena fiocchi","name_en":"Rolled oats","aliases":["Avena fiocchi","Rolled oats","avena fiocchi","rolled oats"],"category":"Carboidrati","kcal":389,"pro":16.9,"carb":66.3,"fat":6.9,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_4","name":"Avena istantanea","name_en":"Instant oats","aliases":["Avena istantanea","Instant oats","avena istantanea","instant oats"],"category":"Carboidrati","kcal":380,"pro":13,"carb":67,"fat":7,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_5","name":"Pasta semola","name_en":"Semolina pasta","aliases":["Pasta semola","Semolina pasta","pasta semola","semolina pasta"],"category":"Carboidrati","kcal":355,"pro":12.5,"carb":73,"fat":1.5,"serving":"100g","unit":"g","rank":5,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_6","name":"Pasta integrale","name_en":"Whole wheat pasta","aliases":["Pasta integrale","Whole wheat pasta","pasta integrale","whole wheat pasta"],"category":"Carboidrati","kcal":348,"pro":13,"carb":67,"fat":2.5,"serving":"100g","unit":"g","rank":6,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_7","name":"Pane bianco","name_en":"White bread","aliases":["Pane bianco","White bread","pane bianco","white bread"],"category":"Carboidrati","kcal":265,"pro":9,"carb":49,"fat":3.2,"serving":"100g","unit":"g","rank":7,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_8","name":"Pane integrale","name_en":"Whole wheat bread","aliases":["Pane integrale","Whole wheat bread","pane integrale","whole wheat bread"],"category":"Carboidrati","kcal":247,"pro":13,"carb":41,"fat":3.4,"serving":"100g","unit":"g","rank":8,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_9","name":"Pane di segale","name_en":"Rye bread","aliases":["Pane di segale","Rye bread","pane di segale","rye bread"],"category":"Carboidrati","kcal":259,"pro":9,"carb":48,"fat":3.3,"serving":"100g","unit":"g","rank":9,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_10","name":"Patate","name_en":"Potato","aliases":["Patate","Potato","patate","potato"],"category":"Carboidrati","kcal":77,"pro":2,"carb":17,"fat":0.1,"serving":"100g","unit":"g","rank":10,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_11","name":"Patate dolci","name_en":"Sweet potato","aliases":["Patate dolci","Sweet potato","patate dolci","sweet potato"],"category":"Carboidrati","kcal":86,"pro":1.6,"carb":20.1,"fat":0.1,"serving":"100g","unit":"g","rank":11,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_12","name":"Gallette di riso","name_en":"Rice cakes","aliases":["Gallette di riso","Rice cakes","gallette di riso","rice cakes"],"category":"Carboidrati","kcal":380,"pro":8,"carb":82,"fat":1.5,"serving":"100g","unit":"g","rank":12,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_13","name":"Couscous","name_en":"Couscous","aliases":["Couscous","couscous"],"category":"Carboidrati","kcal":376,"pro":13,"carb":77,"fat":0.6,"serving":"100g","unit":"g","rank":13,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_14","name":"Quinoa","name_en":"Quinoa","aliases":["Quinoa","quinoa"],"category":"Carboidrati","kcal":368,"pro":14,"carb":64,"fat":6,"serving":"100g","unit":"g","rank":14,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_15","name":"Bulgur","name_en":"Bulgur","aliases":["Bulgur","bulgur"],"category":"Carboidrati","kcal":342,"pro":12,"carb":76,"fat":1.3,"serving":"100g","unit":"g","rank":15,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_16","name":"Farro","name_en":"Farro","aliases":["Farro","farro"],"category":"Carboidrati","kcal":340,"pro":15,"carb":67,"fat":2.5,"serving":"100g","unit":"g","rank":16,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_17","name":"Orzo perlato","name_en":"Pearl barley","aliases":["Orzo perlato","Pearl barley","orzo perlato","pearl barley"],"category":"Carboidrati","kcal":352,"pro":10,"carb":78,"fat":1.2,"serving":"100g","unit":"g","rank":17,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_18","name":"Mais","name_en":"Corn","aliases":["Mais","Corn","mais","corn"],"category":"Carboidrati","kcal":86,"pro":3.3,"carb":19,"fat":1.2,"serving":"100g","unit":"g","rank":18,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_19","name":"Tortilla di mais","name_en":"Corn tortilla","aliases":["Tortilla di mais","Corn tortilla","tortilla di mais","corn tortilla"],"category":"Carboidrati","kcal":218,"pro":5.7,"carb":44,"fat":2.9,"serving":"100g","unit":"g","rank":19,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_20","name":"Wraps","name_en":"Flour tortilla","aliases":["Wraps","Flour tortilla","wraps","flour tortilla"],"category":"Carboidrati","kcal":300,"pro":8,"carb":50,"fat":8,"serving":"100g","unit":"g","rank":20,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_21","name":"Banana","name_en":"Banana","aliases":["Banana","banana"],"category":"Carboidrati","kcal":89,"pro":1.1,"carb":23,"fat":0.3,"serving":"100g","unit":"g","rank":21,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_22","name":"Mela","name_en":"Apple","aliases":["Mela","Apple","mela","apple"],"category":"Carboidrati","kcal":52,"pro":0.3,"carb":14,"fat":0.2,"serving":"100g","unit":"g","rank":22,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_23","name":"Pera","name_en":"Pear","aliases":["Pera","Pear","pera","pear"],"category":"Carboidrati","kcal":57,"pro":0.4,"carb":15,"fat":0.1,"serving":"100g","unit":"g","rank":23,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_24","name":"Arancia","name_en":"Orange","aliases":["Arancia","Orange","arancia","orange"],"category":"Carboidrati","kcal":47,"pro":0.9,"carb":12,"fat":0.1,"serving":"100g","unit":"g","rank":24,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_25","name":"Mirtilli","name_en":"Blueberries","aliases":["Mirtilli","Blueberries","mirtilli","blueberries"],"category":"Carboidrati","kcal":57,"pro":0.7,"carb":14,"fat":0.3,"serving":"100g","unit":"g","rank":25,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_26","name":"Fragole","name_en":"Strawberries","aliases":["Fragole","Strawberries","fragole","strawberries"],"category":"Carboidrati","kcal":32,"pro":0.7,"carb":7.7,"fat":0.3,"serving":"100g","unit":"g","rank":26,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_27","name":"Uva","name_en":"Grapes","aliases":["Uva","Grapes","uva","grapes"],"category":"Carboidrati","kcal":69,"pro":0.7,"carb":18,"fat":0.2,"serving":"100g","unit":"g","rank":27,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_28","name":"Mango","name_en":"Mango","aliases":["Mango","mango"],"category":"Carboidrati","kcal":60,"pro":0.8,"carb":15,"fat":0.4,"serving":"100g","unit":"g","rank":28,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_29","name":"Ananas","name_en":"Pineapple","aliases":["Ananas","Pineapple","ananas","pineapple"],"category":"Carboidrati","kcal":50,"pro":0.5,"carb":13,"fat":0.1,"serving":"100g","unit":"g","rank":29,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_30","name":"Fichi","name_en":"Figs","aliases":["Fichi","Figs","fichi","figs"],"category":"Carboidrati","kcal":74,"pro":0.8,"carb":19,"fat":0.3,"serving":"100g","unit":"g","rank":30,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_31","name":"Datteri","name_en":"Dates","aliases":["Datteri","Dates","datteri","dates"],"category":"Carboidrati","kcal":282,"pro":2.5,"carb":75,"fat":0.4,"serving":"100g","unit":"g","rank":31,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_32","name":"Miele","name_en":"Honey","aliases":["Miele","Honey","miele","honey"],"category":"Carboidrati","kcal":304,"pro":0.3,"carb":82,"fat":0,"serving":"100g","unit":"g","rank":32,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_33","name":"Marmellata","name_en":"Jam","aliases":["Marmellata","Jam","marmellata","jam"],"category":"Carboidrati","kcal":250,"pro":0.4,"carb":65,"fat":0.1,"serving":"100g","unit":"g","rank":33,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_34","name":"Corn flakes","name_en":"Corn flakes","aliases":["Corn flakes","corn flakes"],"category":"Carboidrati","kcal":357,"pro":8,"carb":84,"fat":0.4,"serving":"100g","unit":"g","rank":34,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_35","name":"Muesli","name_en":"Muesli","aliases":["Muesli","muesli"],"category":"Carboidrati","kcal":370,"pro":10,"carb":65,"fat":7,"serving":"100g","unit":"g","rank":35,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_36","name":"Patate lesse","name_en":"Boiled potato","aliases":["Patate lesse","Boiled potato","patate lesse","boiled potato"],"category":"Carboidrati","kcal":87,"pro":1.9,"carb":20,"fat":0.1,"serving":"100g","unit":"g","rank":36,"source":"nurvan_staple_v3"},{"id":"food_Carboidrati_37","name":"Gnocchi","name_en":"Gnocchi","aliases":["Gnocchi","gnocchi"],"category":"Carboidrati","kcal":133,"pro":3.5,"carb":27,"fat":0.5,"serving":"100g","unit":"g","rank":37,"source":"nurvan_staple_v3"},{"id":"food_extra_0","name":"Riso basmati cotto","name_en":"Cooked basmati rice","aliases":["Riso basmati cotto","Cooked basmati rice","riso basmati cotto","cooked basmati rice"],"category":"Carboidrati","kcal":130,"pro":2.7,"carb":28,"fat":0.3,"serving":"100g","unit":"g","rank":100,"source":"nurvan_staple_v3"},{"id":"food_extra_1","name":"Pasta cotta","name_en":"Cooked pasta","aliases":["Pasta cotta","Cooked pasta","pasta cotta","cooked pasta"],"category":"Carboidrati","kcal":158,"pro":5.8,"carb":31,"fat":0.9,"serving":"100g","unit":"g","rank":101,"source":"nurvan_staple_v3"},{"id":"food_extra_6","name":"Hamburger bun","name_en":"Burger bun","aliases":["Hamburger bun","Burger bun","hamburger bun","burger bun"],"category":"Carboidrati","kcal":280,"pro":9,"carb":50,"fat":5,"serving":"100g","unit":"g","rank":106,"source":"nurvan_staple_v3"},{"id":"food_extra_11","name":"Riso","name_en":"Rice / Arroz / Reis / Riz / Рис","aliases":["Riso","Rice / Arroz / Reis / Riz / Рис","riso","rice / arroz / reis / riz / рис"],"category":"Carboidrati","kcal":360,"pro":7,"carb":79,"fat":0.6,"serving":"100g","unit":"g","rank":111,"source":"nurvan_staple_v3"},{"id":"food_extra_12","name":"Avena","name_en":"Oats / Avena / Hafer / Avoine / Овсянка","aliases":["Avena","Oats / Avena / Hafer / Avoine / Овсянка","avena","oats / avena / hafer / avoine / овсянка"],"category":"Carboidrati","kcal":389,"pro":17,"carb":66,"fat":7,"serving":"100g","unit":"g","rank":112,"source":"nurvan_staple_v3"},{"id":"food_extra_16","name":"Pane","name_en":"Bread / Pan / Brot / Pain / Хлеб","aliases":["Pane","Bread / Pan / Brot / Pain / Хлеб","pane","bread / pan / brot / pain / хлеб"],"category":"Carboidrati","kcal":265,"pro":9,"carb":49,"fat":3,"serving":"100g","unit":"g","rank":116,"source":"nurvan_staple_v3"},{"id":"food_extra_23","name":"Pasta","name_en":"Pasta / Nudeln / Pâtes / Макароны","aliases":["Pasta","Pasta / Nudeln / Pâtes / Макароны","pasta","pasta / nudeln / pâtes / макароны"],"category":"Carboidrati","kcal":355,"pro":12,"carb":73,"fat":1.5,"serving":"100g","unit":"g","rank":123,"source":"nurvan_staple_v3"},{"id":"food_Grassi_0","name":"Olio EVO","name_en":"Extra virgin olive oil","aliases":["Olio EVO","Extra virgin olive oil","olio evo","extra virgin olive oil"],"category":"Grassi","kcal":884,"pro":0,"carb":0,"fat":100,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Grassi_1","name":"Olio di cocco","name_en":"Coconut oil","aliases":["Olio di cocco","Coconut oil","olio di cocco","coconut oil"],"category":"Grassi","kcal":862,"pro":0,"carb":0,"fat":100,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Grassi_2","name":"Burro","name_en":"Butter","aliases":["Burro","Butter","burro","butter"],"category":"Grassi","kcal":717,"pro":0.9,"carb":0.1,"fat":81,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Grassi_3","name":"Burro di arachidi","name_en":"Peanut butter","aliases":["Burro di arachidi","Peanut butter","burro di arachidi","peanut butter"],"category":"Grassi","kcal":588,"pro":25,"carb":20,"fat":50,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Grassi_4","name":"Burro di mandorle","name_en":"Almond butter","aliases":["Burro di mandorle","Almond butter","burro di mandorle","almond butter"],"category":"Grassi","kcal":614,"pro":21,"carb":19,"fat":56,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_Grassi_5","name":"Mandorle","name_en":"Almonds","aliases":["Mandorle","Almonds","mandorle","almonds"],"category":"Grassi","kcal":579,"pro":21,"carb":22,"fat":49,"serving":"100g","unit":"g","rank":5,"source":"nurvan_staple_v3"},{"id":"food_Grassi_6","name":"Noci","name_en":"Walnuts","aliases":["Noci","Walnuts","noci","walnuts"],"category":"Grassi","kcal":654,"pro":15,"carb":14,"fat":65,"serving":"100g","unit":"g","rank":6,"source":"nurvan_staple_v3"},{"id":"food_Grassi_7","name":"Nocciole","name_en":"Hazelnuts","aliases":["Nocciole","Hazelnuts","nocciole","hazelnuts"],"category":"Grassi","kcal":628,"pro":15,"carb":17,"fat":61,"serving":"100g","unit":"g","rank":7,"source":"nurvan_staple_v3"},{"id":"food_Grassi_8","name":"Anacardi","name_en":"Cashews","aliases":["Anacardi","Cashews","anacardi","cashews"],"category":"Grassi","kcal":553,"pro":18,"carb":30,"fat":44,"serving":"100g","unit":"g","rank":8,"source":"nurvan_staple_v3"},{"id":"food_Grassi_9","name":"Pistacchi","name_en":"Pistachios","aliases":["Pistacchi","Pistachios","pistacchi","pistachios"],"category":"Grassi","kcal":560,"pro":20,"carb":28,"fat":45,"serving":"100g","unit":"g","rank":9,"source":"nurvan_staple_v3"},{"id":"food_Grassi_10","name":"Semi di chia","name_en":"Chia seeds","aliases":["Semi di chia","Chia seeds","semi di chia","chia seeds"],"category":"Grassi","kcal":486,"pro":17,"carb":42,"fat":31,"serving":"100g","unit":"g","rank":10,"source":"nurvan_staple_v3"},{"id":"food_Grassi_11","name":"Semi di lino","name_en":"Flax seeds","aliases":["Semi di lino","Flax seeds","semi di lino","flax seeds"],"category":"Grassi","kcal":534,"pro":18,"carb":29,"fat":42,"serving":"100g","unit":"g","rank":11,"source":"nurvan_staple_v3"},{"id":"food_Grassi_12","name":"Semi di zucca","name_en":"Pumpkin seeds","aliases":["Semi di zucca","Pumpkin seeds","semi di zucca","pumpkin seeds"],"category":"Grassi","kcal":559,"pro":30,"carb":11,"fat":49,"serving":"100g","unit":"g","rank":12,"source":"nurvan_staple_v3"},{"id":"food_Grassi_13","name":"Avocado","name_en":"Avocado","aliases":["Avocado","avocado"],"category":"Grassi","kcal":160,"pro":2,"carb":8.5,"fat":14.7,"serving":"100g","unit":"g","rank":13,"source":"nurvan_staple_v3"},{"id":"food_Grassi_14","name":"Olive nere","name_en":"Black olives","aliases":["Olive nere","Black olives","olive nere","black olives"],"category":"Grassi","kcal":115,"pro":0.8,"carb":6,"fat":11,"serving":"100g","unit":"g","rank":14,"source":"nurvan_staple_v3"},{"id":"food_Grassi_15","name":"Cioccolato fondente 85%","name_en":"Dark chocolate 85%","aliases":["Cioccolato fondente 85%","Dark chocolate 85%","cioccolato fondente 85%","dark chocolate 85%"],"category":"Grassi","kcal":598,"pro":8,"carb":28,"fat":46,"serving":"100g","unit":"g","rank":15,"source":"nurvan_staple_v3"},{"id":"food_Grassi_16","name":"Mayonnaise light","name_en":"Light mayo","aliases":["Mayonnaise light","Light mayo","mayonnaise light","light mayo"],"category":"Grassi","kcal":260,"pro":1,"carb":8,"fat":25,"serving":"100g","unit":"g","rank":16,"source":"nurvan_staple_v3"},{"id":"food_Grassi_17","name":"Formaggio spalmabile light","name_en":"Light cream cheese","aliases":["Formaggio spalmabile light","Light cream cheese","formaggio spalmabile light","light cream cheese"],"category":"Grassi","kcal":150,"pro":8,"carb":4,"fat":11,"serving":"100g","unit":"g","rank":17,"source":"nurvan_staple_v3"},{"id":"food_extra_19","name":"Olio oliva","name_en":"Olive oil / Aceite / Olivenöl / Huile d'olive","aliases":["Olio oliva","Olive oil / Aceite / Olivenöl / Huile d'olive","olio oliva","olive oil / aceite / olivenöl / huile d'olive"],"category":"Grassi","kcal":884,"pro":0,"carb":0,"fat":100,"serving":"100g","unit":"g","rank":119,"source":"nurvan_staple_v3"},{"id":"food_Proteine_0","name":"Petto di pollo","name_en":"Chicken breast","aliases":["Petto di pollo","Chicken breast","petto di pollo","chicken breast"],"category":"Proteine","kcal":110,"pro":23,"carb":0,"fat":1.2,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Proteine_1","name":"Petto di tacchino","name_en":"Turkey breast","aliases":["Petto di tacchino","Turkey breast","petto di tacchino","turkey breast"],"category":"Proteine","kcal":105,"pro":24,"carb":0,"fat":1,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Proteine_2","name":"Fesa di tacchino","name_en":"Turkey breast sliced","aliases":["Fesa di tacchino","Turkey breast sliced","fesa di tacchino","turkey breast sliced"],"category":"Proteine","kcal":107,"pro":24,"carb":0,"fat":1.2,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Proteine_3","name":"Albume uovo","name_en":"Egg white","aliases":["Albume uovo","Egg white","albume uovo","egg white"],"category":"Proteine","kcal":52,"pro":11,"carb":0.7,"fat":0.2,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Proteine_4","name":"Uovo intero","name_en":"Whole egg","aliases":["Uovo intero","Whole egg","uovo intero","whole egg"],"category":"Proteine","kcal":143,"pro":12.6,"carb":0.8,"fat":9.5,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_Proteine_5","name":"Bresaola","name_en":"Bresaola","aliases":["Bresaola","bresaola"],"category":"Proteine","kcal":151,"pro":32,"carb":0.5,"fat":2.6,"serving":"100g","unit":"g","rank":5,"source":"nurvan_staple_v3"},{"id":"food_Proteine_6","name":"Prosciutto cotto magro","name_en":"Lean cooked ham","aliases":["Prosciutto cotto magro","Lean cooked ham","prosciutto cotto magro","lean cooked ham"],"category":"Proteine","kcal":120,"pro":20,"carb":1,"fat":4,"serving":"100g","unit":"g","rank":6,"source":"nurvan_staple_v3"},{"id":"food_Proteine_7","name":"Prosciutto crudo","name_en":"Cured ham","aliases":["Prosciutto crudo","Cured ham","prosciutto crudo","cured ham"],"category":"Proteine","kcal":250,"pro":26,"carb":0.5,"fat":16,"serving":"100g","unit":"g","rank":7,"source":"nurvan_staple_v3"},{"id":"food_Proteine_8","name":"Tonno al naturale","name_en":"Tuna in brine","aliases":["Tonno al naturale","Tuna in brine","tonno al naturale","tuna in brine"],"category":"Proteine","kcal":103,"pro":24,"carb":0,"fat":0.8,"serving":"100g","unit":"g","rank":8,"source":"nurvan_staple_v3"},{"id":"food_Proteine_9","name":"Tonno all'olio","name_en":"Tuna in oil","aliases":["Tonno all'olio","Tuna in oil","tonno all'olio","tuna in oil"],"category":"Proteine","kcal":190,"pro":25,"carb":0,"fat":10,"serving":"100g","unit":"g","rank":9,"source":"nurvan_staple_v3"},{"id":"food_Proteine_10","name":"Salmone fresco","name_en":"Fresh salmon","aliases":["Salmone fresco","Fresh salmon","salmone fresco","fresh salmon"],"category":"Proteine","kcal":208,"pro":20,"carb":0,"fat":13,"serving":"100g","unit":"g","rank":10,"source":"nurvan_staple_v3"},{"id":"food_Proteine_11","name":"Salmone affumicato","name_en":"Smoked salmon","aliases":["Salmone affumicato","Smoked salmon","salmone affumicato","smoked salmon"],"category":"Proteine","kcal":175,"pro":22,"carb":0,"fat":9,"serving":"100g","unit":"g","rank":11,"source":"nurvan_staple_v3"},{"id":"food_Proteine_12","name":"Merluzzo","name_en":"Cod","aliases":["Merluzzo","Cod","merluzzo","cod"],"category":"Proteine","kcal":82,"pro":18,"carb":0,"fat":0.7,"serving":"100g","unit":"g","rank":12,"source":"nurvan_staple_v3"},{"id":"food_Proteine_13","name":"Orata","name_en":"Sea bream","aliases":["Orata","Sea bream","orata","sea bream"],"category":"Proteine","kcal":120,"pro":20,"carb":0,"fat":4,"serving":"100g","unit":"g","rank":13,"source":"nurvan_staple_v3"},{"id":"food_Proteine_14","name":"Branzino","name_en":"Sea bass","aliases":["Branzino","Sea bass","branzino","sea bass"],"category":"Proteine","kcal":110,"pro":20,"carb":0,"fat":3,"serving":"100g","unit":"g","rank":14,"source":"nurvan_staple_v3"},{"id":"food_Proteine_15","name":"Gamberi","name_en":"Shrimp","aliases":["Gamberi","Shrimp","gamberi","shrimp"],"category":"Proteine","kcal":99,"pro":24,"carb":0.2,"fat":0.3,"serving":"100g","unit":"g","rank":15,"source":"nurvan_staple_v3"},{"id":"food_Proteine_16","name":"Calamari","name_en":"Squid","aliases":["Calamari","Squid","calamari","squid"],"category":"Proteine","kcal":92,"pro":16,"carb":3,"fat":1.4,"serving":"100g","unit":"g","rank":16,"source":"nurvan_staple_v3"},{"id":"food_Proteine_17","name":"Manzo magro","name_en":"Lean beef","aliases":["Manzo magro","Lean beef","manzo magro","lean beef"],"category":"Proteine","kcal":150,"pro":22,"carb":0,"fat":7,"serving":"100g","unit":"g","rank":17,"source":"nurvan_staple_v3"},{"id":"food_Proteine_18","name":"Macinato magro 5%","name_en":"Lean ground beef 5%","aliases":["Macinato magro 5%","Lean ground beef 5%","macinato magro 5%","lean ground beef 5%"],"category":"Proteine","kcal":140,"pro":20,"carb":0,"fat":6,"serving":"100g","unit":"g","rank":18,"source":"nurvan_staple_v3"},{"id":"food_Proteine_19","name":"Filetto di manzo","name_en":"Beef tenderloin","aliases":["Filetto di manzo","Beef tenderloin","filetto di manzo","beef tenderloin"],"category":"Proteine","kcal":180,"pro":26,"carb":0,"fat":8,"serving":"100g","unit":"g","rank":19,"source":"nurvan_staple_v3"},{"id":"food_Proteine_20","name":"Lonza di maiale","name_en":"Pork loin","aliases":["Lonza di maiale","Pork loin","lonza di maiale","pork loin"],"category":"Proteine","kcal":160,"pro":22,"carb":0,"fat":8,"serving":"100g","unit":"g","rank":20,"source":"nurvan_staple_v3"},{"id":"food_Proteine_21","name":"Petto d'anatra","name_en":"Duck breast","aliases":["Petto d'anatra","Duck breast","petto d'anatra","duck breast"],"category":"Proteine","kcal":200,"pro":19,"carb":0,"fat":14,"serving":"100g","unit":"g","rank":21,"source":"nurvan_staple_v3"},{"id":"food_Proteine_22","name":"Tofu firm","name_en":"Firm tofu","aliases":["Tofu firm","Firm tofu","tofu firm","firm tofu"],"category":"Proteine","kcal":144,"pro":17,"carb":3,"fat":9,"serving":"100g","unit":"g","rank":22,"source":"nurvan_staple_v3"},{"id":"food_Proteine_23","name":"Tempeh","name_en":"Tempeh","aliases":["Tempeh","tempeh"],"category":"Proteine","kcal":193,"pro":19,"carb":9,"fat":11,"serving":"100g","unit":"g","rank":23,"source":"nurvan_staple_v3"},{"id":"food_Proteine_24","name":"Seitan","name_en":"Seitan","aliases":["Seitan","seitan"],"category":"Proteine","kcal":120,"pro":25,"carb":4,"fat":1,"serving":"100g","unit":"g","rank":24,"source":"nurvan_staple_v3"},{"id":"food_Proteine_25","name":"Fiocchi di latte","name_en":"Cottage cheese","aliases":["Fiocchi di latte","Cottage cheese","fiocchi di latte","cottage cheese"],"category":"Proteine","kcal":85,"pro":12,"carb":3.5,"fat":2,"serving":"100g","unit":"g","rank":25,"source":"nurvan_staple_v3"},{"id":"food_Proteine_26","name":"Yogurt greco 0%","name_en":"Greek yogurt 0%","aliases":["Yogurt greco 0%","Greek yogurt 0%","yogurt greco 0%","greek yogurt 0%"],"category":"Proteine","kcal":59,"pro":10,"carb":3.6,"fat":0.4,"serving":"100g","unit":"g","rank":26,"source":"nurvan_staple_v3"},{"id":"food_Proteine_27","name":"Yogurt greco intero","name_en":"Greek yogurt full fat","aliases":["Yogurt greco intero","Greek yogurt full fat","yogurt greco intero","greek yogurt full fat"],"category":"Proteine","kcal":97,"pro":9,"carb":3.6,"fat":5,"serving":"100g","unit":"g","rank":27,"source":"nurvan_staple_v3"},{"id":"food_Proteine_28","name":"Skyr","name_en":"Skyr","aliases":["Skyr","skyr"],"category":"Proteine","kcal":63,"pro":11,"carb":4,"fat":0.2,"serving":"100g","unit":"g","rank":28,"source":"nurvan_staple_v3"},{"id":"food_Proteine_29","name":"Ricotta light","name_en":"Light ricotta","aliases":["Ricotta light","Light ricotta","ricotta light","light ricotta"],"category":"Proteine","kcal":110,"pro":11,"carb":3,"fat":6,"serving":"100g","unit":"g","rank":29,"source":"nurvan_staple_v3"},{"id":"food_Proteine_30","name":"Parmigiano","name_en":"Parmesan","aliases":["Parmigiano","Parmesan","parmigiano","parmesan"],"category":"Proteine","kcal":392,"pro":33,"carb":3,"fat":28,"serving":"100g","unit":"g","rank":30,"source":"nurvan_staple_v3"},{"id":"food_Proteine_31","name":"Mozzarella light","name_en":"Light mozzarella","aliases":["Mozzarella light","Light mozzarella","mozzarella light","light mozzarella"],"category":"Proteine","kcal":180,"pro":22,"carb":2,"fat":10,"serving":"100g","unit":"g","rank":31,"source":"nurvan_staple_v3"},{"id":"food_Proteine_32","name":"Whey isolate","name_en":"Whey protein isolate","aliases":["Whey isolate","Whey protein isolate","whey isolate","whey protein isolate"],"category":"Proteine","kcal":370,"pro":90,"carb":1.5,"fat":1,"serving":"100g","unit":"g","rank":32,"source":"nurvan_staple_v3"},{"id":"food_Proteine_33","name":"Caseina","name_en":"Casein protein","aliases":["Caseina","Casein protein","caseina","casein protein"],"category":"Proteine","kcal":360,"pro":80,"carb":5,"fat":1.5,"serving":"100g","unit":"g","rank":33,"source":"nurvan_staple_v3"},{"id":"food_Proteine_34","name":"Edamame","name_en":"Edamame","aliases":["Edamame","edamame"],"category":"Proteine","kcal":122,"pro":11,"carb":10,"fat":5,"serving":"100g","unit":"g","rank":34,"source":"nurvan_staple_v3"},{"id":"food_Proteine_35","name":"Lenticchie cotte","name_en":"Cooked lentils","aliases":["Lenticchie cotte","Cooked lentils","lenticchie cotte","cooked lentils"],"category":"Proteine","kcal":116,"pro":9,"carb":20,"fat":0.4,"serving":"100g","unit":"g","rank":35,"source":"nurvan_staple_v3"},{"id":"food_Proteine_36","name":"Ceci cotti","name_en":"Cooked chickpeas","aliases":["Ceci cotti","Cooked chickpeas","ceci cotti","cooked chickpeas"],"category":"Proteine","kcal":164,"pro":9,"carb":27,"fat":2.6,"serving":"100g","unit":"g","rank":36,"source":"nurvan_staple_v3"},{"id":"food_Proteine_37","name":"Fagioli neri cotti","name_en":"Cooked black beans","aliases":["Fagioli neri cotti","Cooked black beans","fagioli neri cotti","cooked black beans"],"category":"Proteine","kcal":132,"pro":9,"carb":24,"fat":0.5,"serving":"100g","unit":"g","rank":37,"source":"nurvan_staple_v3"},{"id":"food_Proteine_38","name":"White fish","name_en":"White fish","aliases":["White fish","white fish"],"category":"Proteine","kcal":90,"pro":18,"carb":0,"fat":1,"serving":"100g","unit":"g","rank":38,"source":"nurvan_staple_v3"},{"id":"food_Proteine_39","name":"Sardine","name_en":"Sardines","aliases":["Sardine","Sardines","sardine","sardines"],"category":"Proteine","kcal":208,"pro":25,"carb":0,"fat":11,"serving":"100g","unit":"g","rank":39,"source":"nurvan_staple_v3"},{"id":"food_extra_3","name":"Petto di pollo cotto","name_en":"Cooked chicken breast","aliases":["Petto di pollo cotto","Cooked chicken breast","petto di pollo cotto","cooked chicken breast"],"category":"Proteine","kcal":165,"pro":31,"carb":0,"fat":3.6,"serving":"100g","unit":"g","rank":103,"source":"nurvan_staple_v3"},{"id":"food_extra_4","name":"Manzo brasato","name_en":"Braised beef","aliases":["Manzo brasato","Braised beef","manzo brasato","braised beef"],"category":"Proteine","kcal":210,"pro":28,"carb":0,"fat":11,"serving":"100g","unit":"g","rank":104,"source":"nurvan_staple_v3"},{"id":"food_extra_14","name":"Uovo","name_en":"Egg / Huevo / Ei / Œuf / Яйцо","aliases":["Uovo","Egg / Huevo / Ei / Œuf / Яйцо","uovo","egg / huevo / ei / œuf / яйцо"],"category":"Proteine","kcal":143,"pro":13,"carb":1,"fat":10,"serving":"100g","unit":"g","rank":114,"source":"nurvan_staple_v3"},{"id":"food_extra_18","name":"Yogurt","name_en":"Yogurt / Joghurt / Yaourt / Йогурт","aliases":["Yogurt","Yogurt / Joghurt / Yaourt / Йогурт","yogurt","yogurt / joghurt / yaourt / йогурт"],"category":"Proteine","kcal":59,"pro":10,"carb":4,"fat":0.4,"serving":"100g","unit":"g","rank":118,"source":"nurvan_staple_v3"},{"id":"food_extra_20","name":"Tonno","name_en":"Tuna / Atún / Thunfisch / Thon","aliases":["Tonno","Tuna / Atún / Thunfisch / Thon","tonno","tuna / atún / thunfisch / thon"],"category":"Proteine","kcal":103,"pro":24,"carb":0,"fat":1,"serving":"100g","unit":"g","rank":120,"source":"nurvan_staple_v3"},{"id":"food_extra_21","name":"Salmone","name_en":"Salmon / Salmón / Lachs / Saumon","aliases":["Salmone","Salmon / Salmón / Lachs / Saumon","salmone","salmon / salmón / lachs / saumon"],"category":"Proteine","kcal":208,"pro":20,"carb":0,"fat":13,"serving":"100g","unit":"g","rank":121,"source":"nurvan_staple_v3"},{"id":"food_extra_24","name":"Formaggio","name_en":"Cheese / Queso / Käse / Fromage / Сыр","aliases":["Formaggio","Cheese / Queso / Käse / Fromage / Сыр","formaggio","cheese / queso / käse / fromage / сыр"],"category":"Proteine","kcal":350,"pro":25,"carb":2,"fat":27,"serving":"100g","unit":"g","rank":124,"source":"nurvan_staple_v3"},{"id":"food_Snack_0","name":"Barretta proteica","name_en":"Protein bar","aliases":["Barretta proteica","Protein bar","barretta proteica","protein bar"],"category":"Snack","kcal":350,"pro":30,"carb":30,"fat":10,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Snack_1","name":"Crackers integrali","name_en":"Whole grain crackers","aliases":["Crackers integrali","Whole grain crackers","crackers integrali","whole grain crackers"],"category":"Snack","kcal":420,"pro":10,"carb":65,"fat":12,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Snack_2","name":"Popcorn air","name_en":"Air popped popcorn","aliases":["Popcorn air","Air popped popcorn","popcorn air","air popped popcorn"],"category":"Snack","kcal":387,"pro":13,"carb":78,"fat":4.5,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Snack_3","name":"Gelato light","name_en":"Light ice cream","aliases":["Gelato light","Light ice cream","gelato light","light ice cream"],"category":"Snack","kcal":150,"pro":4,"carb":25,"fat":4,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Snack_4","name":"Yogurt drink","name_en":"Yogurt drink","aliases":["Yogurt drink","yogurt drink"],"category":"Snack","kcal":70,"pro":3,"carb":12,"fat":1,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_extra_5","name":"Pizza margherita","name_en":"Margherita pizza","aliases":["Pizza margherita","Margherita pizza","pizza margherita","margherita pizza"],"category":"Snack","kcal":270,"pro":11,"carb":33,"fat":10,"serving":"100g","unit":"g","rank":105,"source":"nurvan_staple_v3"},{"id":"food_extra_7","name":"Ketchup","name_en":"Ketchup","aliases":["Ketchup","ketchup"],"category":"Snack","kcal":112,"pro":1.8,"carb":26,"fat":0.1,"serving":"100g","unit":"g","rank":107,"source":"nurvan_staple_v3"},{"id":"food_extra_8","name":"Senape","name_en":"Mustard","aliases":["Senape","Mustard","senape","mustard"],"category":"Snack","kcal":66,"pro":4,"carb":5,"fat":4,"serving":"100g","unit":"g","rank":108,"source":"nurvan_staple_v3"},{"id":"food_extra_9","name":"Salsa di soia","name_en":"Soy sauce","aliases":["Salsa di soia","Soy sauce","salsa di soia","soy sauce"],"category":"Snack","kcal":53,"pro":8,"carb":5,"fat":0.1,"serving":"100g","unit":"g","rank":109,"source":"nurvan_staple_v3"},{"id":"food_extra_29","name":"Cioccolato","name_en":"Chocolate / Schokolade / Chocolat","aliases":["Cioccolato","Chocolate / Schokolade / Chocolat","cioccolato","chocolate / schokolade / chocolat"],"category":"Snack","kcal":546,"pro":8,"carb":45,"fat":31,"serving":"100g","unit":"g","rank":129,"source":"nurvan_staple_v3"},{"id":"food_Verdure_0","name":"Zucchine","name_en":"Zucchini","aliases":["Zucchine","Zucchini","zucchine","zucchini"],"category":"Verdure","kcal":17,"pro":1.2,"carb":3.1,"fat":0.3,"serving":"100g","unit":"g","rank":0,"source":"nurvan_staple_v3"},{"id":"food_Verdure_1","name":"Carote","name_en":"Carrots","aliases":["Carote","Carrots","carote","carrots"],"category":"Verdure","kcal":41,"pro":0.9,"carb":10,"fat":0.2,"serving":"100g","unit":"g","rank":1,"source":"nurvan_staple_v3"},{"id":"food_Verdure_2","name":"Pomodoro","name_en":"Tomato","aliases":["Pomodoro","Tomato","pomodoro","tomato"],"category":"Verdure","kcal":18,"pro":0.9,"carb":3.9,"fat":0.2,"serving":"100g","unit":"g","rank":2,"source":"nurvan_staple_v3"},{"id":"food_Verdure_3","name":"Lattuga","name_en":"Lettuce","aliases":["Lattuga","Lettuce","lattuga","lettuce"],"category":"Verdure","kcal":15,"pro":1.4,"carb":2.9,"fat":0.2,"serving":"100g","unit":"g","rank":3,"source":"nurvan_staple_v3"},{"id":"food_Verdure_4","name":"Spinaci","name_en":"Spinach","aliases":["Spinaci","Spinach","spinaci","spinach"],"category":"Verdure","kcal":23,"pro":2.9,"carb":3.6,"fat":0.4,"serving":"100g","unit":"g","rank":4,"source":"nurvan_staple_v3"},{"id":"food_Verdure_5","name":"Broccoli","name_en":"Broccoli","aliases":["Broccoli","broccoli"],"category":"Verdure","kcal":34,"pro":2.8,"carb":7,"fat":0.4,"serving":"100g","unit":"g","rank":5,"source":"nurvan_staple_v3"},{"id":"food_Verdure_6","name":"Cavolfiore","name_en":"Cauliflower","aliases":["Cavolfiore","Cauliflower","cavolfiore","cauliflower"],"category":"Verdure","kcal":25,"pro":1.9,"carb":5,"fat":0.3,"serving":"100g","unit":"g","rank":6,"source":"nurvan_staple_v3"},{"id":"food_Verdure_7","name":"Peperoni","name_en":"Bell peppers","aliases":["Peperoni","Bell peppers","peperoni","bell peppers"],"category":"Verdure","kcal":31,"pro":1,"carb":6,"fat":0.3,"serving":"100g","unit":"g","rank":7,"source":"nurvan_staple_v3"},{"id":"food_Verdure_8","name":"Cetriolo","name_en":"Cucumber","aliases":["Cetriolo","Cucumber","cetriolo","cucumber"],"category":"Verdure","kcal":16,"pro":0.7,"carb":3.6,"fat":0.1,"serving":"100g","unit":"g","rank":8,"source":"nurvan_staple_v3"},{"id":"food_Verdure_9","name":"Melanzane","name_en":"Eggplant","aliases":["Melanzane","Eggplant","melanzane","eggplant"],"category":"Verdure","kcal":25,"pro":1,"carb":6,"fat":0.2,"serving":"100g","unit":"g","rank":9,"source":"nurvan_staple_v3"},{"id":"food_Verdure_10","name":"Asparagi","name_en":"Asparagus","aliases":["Asparagi","Asparagus","asparagi","asparagus"],"category":"Verdure","kcal":20,"pro":2.2,"carb":3.9,"fat":0.1,"serving":"100g","unit":"g","rank":10,"source":"nurvan_staple_v3"},{"id":"food_Verdure_11","name":"Fagiolini","name_en":"Green beans","aliases":["Fagiolini","Green beans","fagiolini","green beans"],"category":"Verdure","kcal":31,"pro":1.8,"carb":7,"fat":0.1,"serving":"100g","unit":"g","rank":11,"source":"nurvan_staple_v3"},{"id":"food_Verdure_12","name":"Cavolo nero","name_en":"Kale","aliases":["Cavolo nero","Kale","cavolo nero","kale"],"category":"Verdure","kcal":49,"pro":4.3,"carb":9,"fat":0.9,"serving":"100g","unit":"g","rank":12,"source":"nurvan_staple_v3"},{"id":"food_Verdure_13","name":"Rucola","name_en":"Arugula","aliases":["Rucola","Arugula","rucola","arugula"],"category":"Verdure","kcal":25,"pro":2.6,"carb":3.7,"fat":0.7,"serving":"100g","unit":"g","rank":13,"source":"nurvan_staple_v3"},{"id":"food_Verdure_14","name":"Funghi","name_en":"Mushrooms","aliases":["Funghi","Mushrooms","funghi","mushrooms"],"category":"Verdure","kcal":22,"pro":3.1,"carb":3.3,"fat":0.3,"serving":"100g","unit":"g","rank":14,"source":"nurvan_staple_v3"},{"id":"food_Verdure_15","name":"Cipolla","name_en":"Onion","aliases":["Cipolla","Onion","cipolla","onion"],"category":"Verdure","kcal":40,"pro":1.1,"carb":9.3,"fat":0.1,"serving":"100g","unit":"g","rank":15,"source":"nurvan_staple_v3"},{"id":"food_Verdure_16","name":"Aglio","name_en":"Garlic","aliases":["Aglio","Garlic","aglio","garlic"],"category":"Verdure","kcal":149,"pro":6.4,"carb":33,"fat":0.5,"serving":"100g","unit":"g","rank":16,"source":"nurvan_staple_v3"},{"id":"food_Verdure_17","name":"Insalata mista","name_en":"Mixed salad","aliases":["Insalata mista","Mixed salad","insalata mista","mixed salad"],"category":"Verdure","kcal":20,"pro":1.5,"carb":3.5,"fat":0.3,"serving":"100g","unit":"g","rank":17,"source":"nurvan_staple_v3"}];
var FOOD_CATEGORIES = ["Bevande","Carboidrati","Grassi","Proteine","Snack","Verdure"];
...
function searchFoodCatalog(query, limit = 8) {
...
{ FOOD_CATALOG, FOOD_CATEGORIES, searchFoodCatalog };
...
var SPLIT_IDS = ['fullbody', 'monofrequency', 'upper_lower'];
...
function weekProgress(week, duration, model, isDeload) {
...
function rankCatalogPrograms(list, filters = {}) {
...
{ expandScienceProgramWeeks, parseCatalogQuery, rankCatalogPrograms, SPLIT_LABELS, GOAL_LABELS, PROGRESSION_LABELS };
...
var EXERCISE_DICTIONARY = [
  ...
  { keywords: ["kickback tricipiti", "kickback manubri", "kickback manubrio", "kickback con manubri", "kickback con manubrio", "triceps kickback", "tricep kickback", "dumbbell kickback"], normalized: "Kickback tricipiti", muscle: "TRICIPITI", muscles: ["TRICIPITI"] },
  { keywords: ["kickback cavo tricipiti", "kickback tricipiti al cavo", "kickback tricipiti ai cavi", "cable triceps kickback", "cable tricep kickback"], normalized: "Kickback cavo tricipiti", muscle: "TRICIPITI", muscles: ["TRICIPITI"] },
...
function normalizeExerciseName(rawName) {
  ...
  if (best && /kickback/i.test(best.normalized)) best = kickbackDictionaryEntry(lower) || best;
  if (best) {
    return {
...
function evaluateSimpleExcelFormula(formula, cells) {
  const ctx = cells || {};
...
function sheetToCellMap(sheetName, rawRows) {
  const cells = {};
...
function parseSsttStartInputs(rawRows) {
  const inputs = [];
...
function recalcTrainingLoadsFromCells(weeks, cells, prevCells) {
...
function looksLikeTrainingProgramSheet(rawRows = []) {
...
function matchFoodMacros(foodName) {
...
function enrichFoodItemMacros(item) {
...
function enrichNutritionMacros(nutrition) {
...
function expandProgramToDeclaredDuration(program, targetWeeks) {
...
async function extractPdfPlainTextAsync(buffer) {
  const u8 = toU8(buffer);
  const raw = latin1FromBytes(u8);
  let text = extractPdfTjFromString(raw);
  let chunkCount = 0;
...
async function extractDocxStructured(buffer, filename = "documento.docx") {
