...
export function harvestLabExamRecords(text) {
...
// The length the program declares ("SCHEDA 8 SETTIMANE", "DURATA: 10
// SETTIMANE", "FULL BODY 10/12 SETTIMANE", a file named "... 4 SETTIMANE"):
// in the file name, or on a short heading line. A sentence that mentions
// weeks ("aumentare ogni 2 settimane", "tenute per 3 settimane e poi ruotate")
// declares nothing.
function declaredProgramWeeks(text, filename) {
  const sources = [String(filename || '').replace(/\.[a-z0-9]{2,5}$/i, '')].concat(
    String(text || '').split(/\r?\n/).map((l) => l.trim()).filter((l) => l && !/^_/.test(l) && l.split(/\s+/).length <= 8)
  );
  for (const src of sources) {
    if (/\b(?:ogni|dopo|entro|per\s+le?\s+prime|ultime|a\s+settimane)\b/i.test(src)) continue;
    const slash = src.match(/(\d{1,2})[ \t]*\/[ \t]*(\d{1,2})[ \t]*(?:settimane|weeks)\b/i);
    if (slash) return Math.max(parseInt(slash[1], 10) || 0, parseInt(slash[2], 10) || 0) || null;
    const n = src.match(/\b(\d{1,2})[ \t]*(?:settimane|weeks)\b/i) || src.match(/\b(?:durata|duration|settimane|weeks)[ \t]*[:=][ \t]*(\d{1,2})\b/i);
    if (n) {
...
/**
 * How many weeks the program lasts. What the document declares wins: a
 * 4-week progression in an 8-week program runs twice, a 6-week one in a
 * 4-week program stops at 4. Without a declaration, the longest progression
 * written on one line (3x10-4x10-5x10...) sets the length.
 */
export function inferProgramDurationFromText(text, filename = "", schemeChoices = {}) {
  const declared = declaredProgramWeeks(text, filename);
  if (declared) return Math.min(52, declared);
  let best = 0;
  String(text || "").split(/\r?\n/).forEach((line) => {
